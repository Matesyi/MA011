...
###podil rozptylu
#pomoci kritickeho oboru
f <- rozptyl_X / rozptyl_Y
kvantil1 <- qf(alpha / 2, n_X - 1, n_Y - 1)
kvantil2 <- qf(1 - (alpha / 2), n_X - 1, n_Y - 1)
kvantil1
kvantil2
f
# f se nenachazi v kritickem oboru -> H0 nezamitame

#pomoci p hodnoty (p hodnota = 0.6474)
var.test (X, Y)
...
# druhy zpusob (p hodnota)
var.test (X, Y, var.equal = TRUE)

#treti zpusob (interval spolehlivosti)
kavantil <- qt (1 - alpha/2, n_X + n_Y - 2)
